...
{
	FILE* pTrapPatternFile = fopen(pTrapPatternScript, "r");

	if (pTrapPatternFile == NULL)
	{// 読み込み失敗
		return E_FAIL;
	}
	...
	char aStrCpy[MAX_STRING] = {};		   // 文字列複製(整理)
	char* pStart = NULL;				   // 文字列開始位置
	TrapPattern TrapPattern = {};		   // トラップパターン情報
	int type;

	while (true)
	{
		memset(aStr, NULL, sizeof(aStr));					// 文字列クリア
		(void)fgets(aStr, sizeof(aStr), pTrapPatternFile);	// 一列読み込み
	...
	while (true)
	{
		...
		if (strcmp(aStrCpy, LOAD_PATTERN) == 0)
		{
			while (true)
			{
				...
				if (strcmp(aStrCpy, LOAD_TRAP) == 0)
				{
					while (true)
					{
						...
						{// POSを読み込んだ
							...
							(void)sscanf(pStart + 1, "%f %f %f", &TrapPattern.aTrapInfo[TrapPattern.nNumTrap].pos.x, 
								&TrapPattern.aTrapInfo[TrapPattern.nNumTrap].pos.y,
								&TrapPattern.aTrapInfo[TrapPattern.nNumTrap].pos.z);
						...
						{// ROTを読み込んだ
							...
							(void)sscanf(pStart + 1, "%f %f %f", &TrapPattern.aTrapInfo[TrapPattern.nNumTrap].rot.x,
								&TrapPattern.aTrapInfo[TrapPattern.nNumTrap].rot.y, 
								&TrapPattern.aTrapInfo[TrapPattern.nNumTrap].rot.z);
						...
						{// TYPEを読み込んだ
							...
							TrapPattern.aTrapInfo[TrapPattern.nNumTrap].traptype = (TRAPTYPE)type;

							continue;
						}

						if (strcmp(aStrCpy, LOAD_ENDTRAP) == 0)
						{
							TrapPattern.nNumTrap++;
							break;
						}
					}
				}
				...
				{
					SetTrapPattern(TrapPattern);
					ZeroMemory(&TrapPattern, sizeof(TrapPattern));
					break;
				}
			}
		}
